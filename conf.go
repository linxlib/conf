...
const (
	// DefaultFilename is the default filename of the config file that conf looks for.
	DefaultFilename = "config.toml"
	// DefaultDir is the default directory that conf searches in for the config file.
	DefaultDir = "."
	// DefaultTag is the default struct tag key that conf uses to find the field's alt
	// name.
	DefaultTag = "conf"
	// DefaultTimeLayout is the default time layout that conf uses to parse times.
...
//
// By default conf looks for a file `config.yaml` in the current directory and
// uses the struct field tag `conf` for matching field names and validation.
...
func (f *conf) Load(cfg interface{}) error {
	if !isStructPtr(cfg) {
...
func (f *conf) findCfgFile() (path string, err error) {
...
func (f *conf) decodeFile(file string) (map[string]interface{}, error) {
	fd, err := os.Open(file)
	if err != nil {
		return nil, err
	}
...
func (f *conf) decodeMap(m map[string]interface{}, result interface{}) error {
	dec, err := NewDecoder(&DecoderConfig{
		...
		DecodeHook: ComposeDecodeHookFunc(
			...
			StringToTimeHookFunc(f.timeLayout),
...
// where applicable.
func (f *conf) processCfg(cfg interface{}) error {
...
func (f *conf) processField(field *field) error {
...
func (f *conf) setFromEnv(fv reflect.Value, key string) error {
...
func (f *conf) formatEnvKey(key string) string {
...
func (f *conf) setValue(fv reflect.Value, val string) error {
	switch fv.Kind() {
	case reflect.Ptr:
		if fv.IsNil() {
...
func (f *conf) setSlice(sv reflect.Value, val string) error {
	ss := stringSlice(val)
